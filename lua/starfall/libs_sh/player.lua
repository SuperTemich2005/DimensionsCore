...
function player_methods:getEyeTrace()
    local trace = util.TraceLine({
        start = getply(self):EyePos(),
        endpos = getply(self):EyePos()+getply(self):GetAimVector()*32767,
        filter = function(ent)
            if ent == getply(self) or ent:GetDimension() ~= getply(self):GetDimension() then
                return false
            end
            return true
        end
    })
    return SF.StructWrapper(instance,trace,"TraceResult")
