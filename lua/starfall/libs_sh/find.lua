...
local function convert(results, func)
	...
	if func~=nil then
		...
		for i = 1, #results do
			...
			if e and func(e) and instance.entity:GetDimension() == e:GetDimension() then
				t[#t + 1] = e
			end
		end
	else
		for i = 1, #results do
			local e = owrap(results[i])
			if e and e:GetDimension() == instance.entity:GetDimension() then
				t[#t + 1] = e
			end
		end
	end
...
local function convertIter(iter, func)
	...
	if func~=nil then
		...
		for _, v in iter() do
			local e = owrap(v)
			if e and func(e)and e:GetDimension() == instance.entity:GetDimension() then
				t[#t + 1] = e
			end
		end
	else
...
function find_library.playersByName(name, casesensitive, exact)
	...
	if exact then
		for k, ply in ipairs(player.GetAll()) do
			if getName(ply) == name and ply:GetDimension() == instance.entity:GetDimension() then
				ret[#ret+1] = plywrap(ply)
			end
		end
	else
		for k, ply in ipairs(player.GetAll()) do
			if string.find(getName(ply), name, 1, true) and ply:GetDimension() == instance.entity:GetDimension() then
				ret[#ret+1] = plywrap(ply)
			end
		end
	end
...
function find_library.playerBySteamID(steamid)
	...
	if found and found:GetDimension() == instance.entity:GetDimension() then return plywrap(found) end
end
